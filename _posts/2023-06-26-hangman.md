...
Going step by step, here are my approach to each one.

Loading of the list is quite easy, simple function can load the whole file and put it into an array. Only complication was to load the file as unicode (utf-8).
...
For random picking of one of them, there is handy module [random][1] and its `random.choice`. In terms of code this is something like
